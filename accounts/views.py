...
def logout_view(request):
	print "logging out"
	logout(request)
	messages.success(request, "hanks you for your visiting. See you again.")
...
def login_view(request):
	...
	if form.is_valid():
		...
		login(request, user)
		messages.success(request, "Welcome Back, " + username + " Enjoy your shopping")
		return HttpResponseRedirect("/")
	context = {
		"form": form,
		"submit_btn": btn,
...
def registration_view(request):
	...
	if form.is_valid():
		...
		new_user.save()
		messages.success(request, "Successfully Registered. An activation email have been sent. Please check and activate.")
		return HttpResponseRedirect("/")
		username = form.cleaned_data['username']
		password = form.cleaned_data['password']
		user = authenticate(username=username, password=password)
